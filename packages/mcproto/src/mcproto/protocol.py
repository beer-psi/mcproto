...
class Transform(Protocol):
    """A generic data transformation. Think something like an ASGI middleware."""

    def receive_data(self, data: bytes) -> None: ...
    def process_buffer(self) -> bytes | None: ...


class DecryptionHandler(Transform):
    """
    ...
    def receive_data(self, data: bytes):
        ...
        return self._transform.receive_data(data)

    @override
    def process_buffer(self) -> bytes | None:
        return self._transform.process_buffer()


class DecompressionHandler(Transform):
    """
    ...
    @override
    def process_buffer(self) -> bytes | None:
        if (packet := self._transform.process_buffer()) is None:
            return None
